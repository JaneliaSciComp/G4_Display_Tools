...
%% L%oad .mat file containing user defined settings. These include:
    ...
    %specify time ranges for parsing and data analysis
    data_rate = settings.data_rate; % rate (in Hz) which all data will be aligned to
    pre_dur = settings.pre_dur; %seconds before start of trial to include
    post_dur = settings.post_dur; %seconds after end of trial to include
    da_start = settings.da_start; %seconds after start of trial to start data analysis
    da_stop = settings.da_stop; %seconds before end of trial to end data analysis
    time_conv = settings.time_conv; %converts seconds to microseconds (TDMS timestamps are in micros)
    common_cond_dur = settings.common_cond_dur; %sets whether all condition durations are the same (1) or not (0), for error-checking
    processed_file_name = settings.processed_file_name;
    hist_datatypes = settings.hist_datatypes; %{'Frame Position', 'LmR', 'LpR'};
    ...
    ts_data_normalized = normalize_ts_data(settings.channel_order, ts_data, maxs);
