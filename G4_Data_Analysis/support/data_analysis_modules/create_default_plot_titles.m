function [OL_cond_name] = create_default_plot_titles(conds, cond_name, g4ppath)
    
    %get files in directory
    
    
    OL_cond_name = cell(1,length(conds));    
    %If the .g4p file wasn't found, give all graphs blank names. 
    if isempty(g4ppath)
        
        for i = 1:length(OL_cond_name)
            for j = 1:size(conds{i},1)
                for k = 1:size(conds{i},2)
            ...
            for j = 1:size(conds{i},1)
                for k = 1:size(conds{i},2)
                    if isempty(cond_name)
                        ...
                        if isnan(conds{i}(j,k)) || conds{i}(j,k) == 0
                            continue;
                        end
                        patname = exp.exp_parameters.block_trials{conds{i}(j,k),2};
                        if ~isempty(exp.exp_parameters.block_trials{conds{i}(j,k),3})
                            funcname = exp.exp_parameters.block_trials{conds{i}(j,k),3};
                        else
                            funcname = '';
                        end
