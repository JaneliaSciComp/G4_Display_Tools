...
if nargin<5 
    ...
    OL_conds_vec = find(conditionModes~=4); 
    if ~isempty(OL_conds_vec)
    ...
    CL_conds_vec = find(conditionModes==4);
    if ~isempty(CL_conds_vec)
...
for d = 1:num_CL_datatypes
    CL_inds(d) = find(strcmpi(channelNames.histograms,CL_datatypes{d}));
end
            ...
            for row = 1:num_plot_rows
                for col = 1:num_plot_cols
                    ...
                    if cond>0 
                        ...
                        tmpdata = circshift(squeeze(histograms(d,cond,:,:)),[1 floor(num_positions/2)]);
