function G4_Plot_Data_walkingsensor(exp_folder, trial_options, metadata_for_publishing, CL_conds, OL_conds, TC_conds, overlap)
%FUNCTION G4_Plot_Data_walkingsensor(exp_folder, trial_options, CL_conds, OL_conds, TC_conds, overlap)
% 
% Inputs:
...
if ~isempty(CL_conds)
    ...
    for d = 1:length(CL_inds)
        for fig = 1:num_figs
            ...
            for row = 1:num_plot_rows
                for col = 1:num_plot_cols
                    ...
                    if cond>0
                        ...
                        tmpdata = circshift(squeeze(Data.histograms(d,cond,:,:)),[1 ceil(num_positions/2)]);
            ...
            snapnow; %Takes only one snapshot of each figure.
        end
    end
end
            ...
            for row = 1:num_plot_rows
                for col = 1:num_plot_cols
                    if row > 1
                        ...
                        cond = OL_conds(row*(1+overlap),col,fig);
                    end
                    if ~isnan(cond)
                        better_subplot(num_plot_rows, num_plot_cols, col+num_plot_cols*(row-1))
...
if ~isempty(TC_conds)
    ...
    for d = TC_inds
        for fig = 1:num_figs
            ...
            for row = 1:num_plot_rows
                conds = TC_conds(row*(1+overlap),:,fig);
                conds(isnan(conds)) = [];
